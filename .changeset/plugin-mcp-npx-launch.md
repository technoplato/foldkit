...
Bundle the published `foldkit-devtools-mcp` bin at build time so `dist/server.js` inlines `effect`, the Foldkit devtools protocol, the MCP SDK, and `ws`. The bin now runs standalone under `npx` with no peer dependency resolution. The `foldkit-skills` plugin MCP config launches the server through `npx` with a pinned version instead of pointing at a gitignored build path that never reached plugin installs.
