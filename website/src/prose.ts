...
export const heading = (level: 1 | 2 | 3, text: string): Html => {
  const classes = {
  ...
  return tag[level]([Class(classes[level])], [text])
}
...
export const section = (title: string, content: Html[]): Html =>
  div([], [heading(2, title), ...content])
