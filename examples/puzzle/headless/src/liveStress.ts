...
const adminToken = (): string => {
  ...
  if (token === undefined || token === '') {
    throw new Error(
      'puzzle-headless:live-stress needs INSTANT_APP_ADMIN_TOKEN from the trusted wrapper.',
    )
  }
  return token
}

const puzzleUriPattern = /\/puzzle#[^"'<\s]*/

/** Reads the painted Puzzle URI from a dump or page. */
export const puzzleUriFromDump = (dump: string): Option.Option<string> => {
  const match = puzzleUriPattern.exec(dump)
  if (match === null) {
    return Option.none()
  }
  const raw = Array.get(match, 0)
...
/** Tape length from a painted URI. Empty `#next` is 0. Each `=y` or `=n` is one guess. */
export const tapeLengthFromUri = (uri: string): number => {
  const matches = [...uri.matchAll(/=[yn]/g)]
  return matches.length
}

const tapeLengthFromDump = (dump: string): Option.Option<number> =>
  Option.map(puzzleUriFromDump(dump), tapeLengthFromUri)

/** Reads tape length from an Android uiautomator dump of the Program screen. */
export const countFromAndroidUiDump = (xml: string): Option.Option<number> => {
  ...
  return tapeLengthFromDump(xml)
}

/** Reads tape length from an iOS accessibility dump of the Program screen. */
export const countFromIosDescribe = (dump: string): Option.Option<number> => {
  ...
  return tapeLengthFromDump(dump)
}
...
const tapeLengthFromPage = async (
  page: PlaywrightPage,
): Promise<Option.Option<number>> => {
  try {
    return tapeLengthFromDump(await page.locator('body').innerText())
  } catch {
    return Option.none()
  }
}

const tapeLengthFromTitle = async (
  page: PlaywrightPage,
): Promise<Option.Option<number>> => tapeLengthFromDump(await page.title())

/**
 * Bursts N GuessedYess on the live Instant tape after ResetTape.
 * Every Host tag must catch tape length N. Phone and browser paints
 * are probed when those surfaces are already up.
 */
...
> => {
  const transport = makeAdminPuzzleSnapshotLogTransport(
    FoldkitPuzzleV01.id,
    adminToken(),
  )
  ...
  await waitForCount(writer.readModel, writer.subscribe, target, {
    timeoutMs: liveSettleTimeoutMs,
  })
  ...
  const surfaces = await Promise.all([
    probeBrowser('react', 'http://localhost:5216/', tapeLengthFromPage, target),
    probeBrowser(
      'svelte',
      'http://localhost:5218/',
      tapeLengthFromPage,
...
const printLiveResult = (
  ...
  const clock =
    format === 'ms'
      ? now.toString()
      : formatHeadlessHumanClock(now, process.env['PUZZLE_HEADLESS_TZ'])
...
export const runLiveStressCli = async (): Promise<void> => {
  if (process.env['PUZZLE_LIVE_STRESS'] !== '1') {
    console.error(
      'puzzle-headless:live-stress writes the shared Instant tape. Set PUZZLE_LIVE_STRESS=1.',
    )
    process.exit(1)
  }
  if (process.env['PUZZLE_TAPE'] === 'memory') {
    console.error(
      'puzzle-headless:live-stress uses Instant. Do not set PUZZLE_TAPE=memory.',
    )
    process.exit(1)
  }
