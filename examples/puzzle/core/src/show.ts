...
import { type Model, path, title, uriOf } from './model.js'
import { puzzleValid } from './program.js'

/** Focus among valid Actions. TV chrome can stay on yes. */
export const Focus = S.Literals(['yes', 'no', 'reset'])
/** A focus token. */
...
const pathToken = (path: string | undefined): Option.Option<string> => {
  if (path === undefined || path === 'puzzle') {
    return Option.none()
  }
  if (path.startsWith('puzzle.')) {
    return Option.some(path.slice('puzzle.'.length))
  }
...
export const renderShow = (model: Model, context: ShowContext): string => {
  ...
  return [
    ...
    '',
    'STATE',
    identityField('tape', uriOf(model)),
