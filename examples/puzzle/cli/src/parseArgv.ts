/** Parsed `puzzle` argv. This file must not import Effect or foldkit. */
export type ParsedPuzzleArgv =
  | Readonly<{
      readonly _tag: 'Show'
...
/** Usage printed for `puzzle --help`. */
export const puzzleUsage = [
  'puzzle 0.0.0',
  '',
  'USAGE',
  '',
  '$ puzzle show [--device watch|phone|tablet|computer|tv] [--path PATH]',
  '$ puzzle do <token>',
  '$ puzzle replay --tape <path>',
].join('\n')

/** Parses `puzzle` show / do / replay argv. */
export const parsePuzzleArgv = (
  argv: ReadonlyArray<string>,
): ParsedPuzzleArgv => {
  ...
  if (command === 'do') {
    ...
    if (token === undefined) {
      return {
        _tag: 'Failed',
        message: 'Send yes, no, hint, operator, replicate, or reset.',
        exitCode: 1,
      }
    }
    if (at(argv, 2) !== undefined) {
      return {
        _tag: 'Failed',
        message: `Send one token. Got ${argv.slice(1).join(' ')}. Use yes, no, hint, operator, replicate, or reset.`,
        exitCode: 1,
      }
    }
...
/** Parses `puzzle-screen` argv. Bare invocation shows. */
