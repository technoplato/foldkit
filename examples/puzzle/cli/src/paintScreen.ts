...
const screenBinaryName = 'puzzle-screen'
...
/** Paints the Puzzle screen tree plus its argv vocabulary. */
export const paintPuzzleCli = (model: Model): CliPainting =>
  paintCli(puzzleScreen(model), {
...
export const paintScreenShow = (initialModel: Model): ScreenCliExecution => ({
  ...
  stdout: showStdout(paintPuzzleCli(initialModel)),
  stderr: '',
  exitCode: 0,
})
...
export const resolveScreenDo = (
  ...
  const painting = paintPuzzleCli(initialModel)
...
export const paintScreenDo = (
  ...
  stdout: [
    `sent ${tokenOf(action)}`,
    '',
    showStdout(paintPuzzleCli(nextModel)),
  ].join('\n'),
  stderr: '',
  exitCode: 0,
})
