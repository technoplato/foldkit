...
const visibleMenuRows = (
  ...
  const filtered = filterListedActions(
    listActions(PuzzleProgram, product),
    maybeQuery,
  )
...
const overlayMenuOf = (
  ...
  const filtered = filterListedActions(
    listActions(PuzzleProgram, snapshot.product),
    open.maybeQuery,
  )
...
export const puzzleKeysForToken = (token: string): ReadonlyArray<string> => {
 ...
 */
export const tapForKey = (input: string, handles: PuzzleFactHandles): void => {
 ...
 * Product GuessedYes is not the menu selection path.
 */
