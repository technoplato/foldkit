...
export const view = (model: Model): Document => {
  ...
  const page = Option.match(maybeJob, {
    ...
    onSome: job => {
      ...
      const next = index >= 0 && index < jobs.length - 1 ? jobs[index + 1] : undefined
      const noteLabel = "Job " + String(index + 1) + " of " + String(jobs.length)
      return h.article(
        [h.Class("mx-auto max-w-2xl space-y-6")],
        [
          h.p([h.Class("font-mono text-xs text-sky-800")], [noteLabel + " · " + job.status]),
          h.h1([h.Class("font-serif text-4xl text-stone-900")], [job.title]),
          h.p([h.Class("font-serif text-lg leading-8 text-stone-700")], [job.analysis]),
          h.pre(
            [h.Class("max-h-80 overflow-auto whitespace-pre-wrap font-sans text-sm leading-6")],
            [job.transcriptText.length === 0 ? "Transcript is not ready yet." : job.transcriptText],
          ),
          h.div(
            [h.Class("flex gap-3")],
            [
  ...
  return {
    ...
    body: h.div(
      ...
      [
        h.p([h.Class("mb-8 text-center text-xs uppercase tracking-wide text-sky-800")], ["Knophy transcribe"]),
