...
describe("update", () => {
  ...
  test("ObservedJobs loads Instant jobs", () => {
    ...
    Story.story(
      ...
      Story.model(model => {
        ...
        expect(model.catalog).toEqual(LoadedCatalog.make({ jobs: seedTranscripts }))
      }),
    )
  })
  ...
  test("OpenedHref /?url= selects or queues that video", () => {
    ...
    Story.story(
      ...
      Story.with(initial),
      Story.message(
        OpenedHref.make({ href: "https://transcribe.knophy.com/?url=https://youtu.be/B0FaK0sazXg" }),
      ),
      Story.Command.expectNone(),
      Story.model(model => {
  ...
  test("OpenedHref /v/:id selects that video", () => {
    ...
    Story.story(
      update,
      Story.with(initial),
      Story.message(OpenedHref.make({ href: "https://transcribe.knophy.com/v/B0FaK0sazXg" })),
      Story.Command.expectNone(),
      Story.model(model => {
        expect(model.selectedId).toEqual(Option.some("B0FaK0sazXg"))
      }),
  ...
  test("SubmittedUrl queues an unknown video id", () => {
    ...
    Story.story(
      ...
      Story.model(model => {
        ...
        expect(visibleJobs(model).some(job => job.videoId === "abcdefghijk")).toBe(true)
      }),
    )
  })

  test("videoIdFromInput parses YouTube URLs", () => {
    expect(videoIdFromInput("https://youtu.be/B0FaK0sazXg")).toEqual(Option.some("B0FaK0sazXg"))
    expect(videoIdFromInput("B0FaK0sazXg")).toEqual(Option.some("B0FaK0sazXg"))
    expect(requestFromHref("/v/B0FaK0sazXg").videoId).toEqual(Option.some("B0FaK0sazXg"))
    expect(requestFromHref("/jobs/B0FaK0sazXg").videoId).toEqual(Option.some("B0FaK0sazXg"))
  })
  ...
  test("OpenedHref /jobs/:id selects that video and sets local media", () => {
    ...
    Story.story(
      update,
      Story.with(initial),
      Story.message(OpenedHref.make({ href: "https://transcribe.knophy.com/jobs/B0FaK0sazXg" })),
      Story.Command.expectNone(),
      Story.model(model => {
        expect(model.selectedId).toEqual(Option.some("B0FaK0sazXg"))
  ...
  test("HeardJobPlayback loads words and wordAt uses a half-open interval", () => {
    ...
    Story.story(
      ...
      Story.model(model => {
        ...
        expect(wordAt(model.words, 0).pipe(Option.map(word => word.id))).toEqual(
          Option.some("w0"),
        )
        expect(wordAt(model.words, 0.4).pipe(Option.map(word => word.id))).toEqual(
          Option.some("w1"),
        )
