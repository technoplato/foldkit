...
const titleFromInfo = (infoPath: string | undefined): string => {
  ...
  try {
    const parsed = JSON.parse(readFileSync(infoPath, "utf8")) as { title?: unknown }
...
const whisperTextFromDir = (dir: string): string | undefined => {
  ...
  const jsonName = names.find(name => name.endsWith(".json") && name.includes("whisper"))
...
const framesFromPublic = (videoId: string, transcriptId: string): ReadonlyArray<FrameSpec> => {
  const dir = join(
    import.meta.dirname,
    "..",
    "..",
  ...
  return files.map((name, index) => {
    ...
    return {
      caption: Number.isFinite(tSec) ? `${String(tSec)}s` : `Frame ${String(index + 1)}`,
...
const ingest = async (): Promise<void> => {
  ...
  const artifactDir =
    process.env["TRANSCRIBE_ARTIFACT_DIR"] ?? `/tmp/knophy-transcribe-${SEED_VIDEO_ID}`
  const vtt =
  ...
  const transcriptText =
    whisperText ?? (vtt === undefined ? "" : parseVtt(vtt))
  ...
  const ops = [
    requireTx(database.tx.knophyTranscripts[SEED_ID], "knophyTranscripts").update({
      analysis,
      createdAt: Date.now(),
      slug: SEED_VIDEO_ID,
      status,
    ...
    ...frames.map(frame =>
      requireTx(database.tx.knophyTranscriptFrames[frame.id], "knophyTranscriptFrames").update({
