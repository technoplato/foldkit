...
export type TranscribeInitialRoute = Program.ResolvedProgramRoute<Model, Message>

/** The canonical fresh Transcribe route shared by host carriers. */
const [initialModel] = init()
export const initialTranscribeRoute: TranscribeInitialRoute = Program.state(initialModel)

/** Creates a React Client over host-selected live or deterministic resources. */
export const makeTranscribeReactClient = (resources: Layer.Layer<TranscribeStore>) =>
  ...
  >({
    createActions: enqueueMessage => ({
      ...
      updatedDraftUrl: draftUrl => enqueueMessage(UpdatedDraftUrl.make({ draftUrl })),
    }),
    name: "Transcribe",
...
export const StaticTranscribeClient = makeTranscribeReactClient(StaticTranscribeResources)
