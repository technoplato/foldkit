...
import { makeTranscribeElement } from './element.js'
...
/** Starts the host-owned page and embeds the Transcribe widget. */
export const startHost = (): void => {
  getElement('root').outerHTML = [
    ...
    '<p class="text-sm text-gray-600">This page owns the document title and URL. It embeds Knophy transcribe with Runtime.embed and makeElement.</p>',
    '</header>',
    '<div id="widget-slot"></div>',
    '</div>',
  ].join('')
  const widgetSlot = getElement('widget-slot')
  Runtime.embed(makeTranscribeElement(widgetSlot))
}
