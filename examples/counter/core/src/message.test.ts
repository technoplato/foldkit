...
describe('actions', () => {
  test('Increment is always valid and uses + and =', () => {
    ...
    expect(Increment.valid(Model.make({ count: 0 }), emptyContext)).toBe(true)
    expect(Increment.valid(Model.make({ count: 4 }), emptyContext)).toBe(true)
  })
  ...
  test('Decrement is always valid', () => {
    ...
    expect(Decrement.valid(Model.make({ count: 0 }), emptyContext)).toBe(true)
  })
  ...
  test('Reset is invalid when the count is already 0', () => {
    ...
    expect(Reset.valid(Model.make({ count: 0 }), emptyContext)).toBe(false)
    expect(Reset.hiddenBecause?.(Model.make({ count: 0 }))).toBe(
      'count is already 0',
    )
    expect(Reset.valid(Model.make({ count: 1 }), emptyContext)).toBe(true)
    expect(Reset.hiddenBecause?.(Model.make({ count: 1 }))).toBeUndefined()
  })
