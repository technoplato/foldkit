...
import { ingestScreen } from 'ingest-core-example'
...
export const paintOpenTui = (
  ctx: RenderContext,
  model: Parameters<typeof ingestScreen>[0],
  options: PaintOpenTuiOptions,
): Renderable => paintOpenTuiNode(ctx, ingestScreen(model), options)
...
export const paintOpenTuiFrame = (
  ...
  frame.add(paintOpenTuiNode(ctx, ingestScreen(model), options))
