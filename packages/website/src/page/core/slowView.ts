  ...
  return h.div(
    ...
    [
      ...
      para(
        ...
        inlineCode('view'),
        ' alone takes that long, you are already dropping frames before the DOM work even begins. The warning nudges you to memoize expensive subtrees with ',
        ...
        '. Cache the computation on the model in ',
        inlineCode('update'),
        ' only when memoization cannot cover it, since every update branch that changes its inputs then has to keep that derived field in sync.',
      ),
      para(
