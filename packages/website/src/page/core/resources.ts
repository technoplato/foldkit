  ...
  return h.div(
    ...
    [
      ...
      infoCallout(
        ...
        'Resources are kitchen equipment: the oven, the stand mixer, the deep fryer. They’re turned on when the kitchen opens and run all night. Every dish (Command) can use them. You don’t buy a new oven per order. An ',
        inlineCode('RpcClient'),
        ' and an analytics client are the same: expensive singletons that live for the entire app lifecycle. Need multiple pieces of equipment? Combine them with ',
      ...
      bullets(
        ...
        h.span(
          ...
          [
            ...
            ' A per-Command provide constructs a fresh instance for each invocation, so Commands never share state through it. When every Command must talk to the same object, like one ',
            inlineCode('RpcClient'),
            ' that multiplexes their calls over a single connection, the service belongs in ',
            inlineCode('resources'),
            '.',
          ],
        ),
