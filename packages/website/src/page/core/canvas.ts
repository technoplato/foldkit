...
export const view = (copiedSnippets: CopiedSnippets): Html =>
  h.div(
    ...
    [
      ...
      para(
        inlineCode('Canvas.animationFrame'),
        ' is a ',
        link(coreSubscriptionsRouter(), 'Subscription'),
        ' helper that emits a Message every ',
        inlineCode('requestAnimationFrame'),
        ' tick, with the inter-frame delta in milliseconds. The ',
        inlineCode('isActive'),
        ' callback gates the loop on Model state: when it returns ',
        inlineCode('false'),
        ' the rAF loop tears down entirely, and Foldkit restarts it when the gate flips back. Pair with ',
