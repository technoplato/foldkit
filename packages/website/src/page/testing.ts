...
export const view = (copiedSnippets: CopiedSnippets): Html => {
  ...
  return h.div(
    ...
    [
      ...
      para(
        'Name test files for their test style, beside the code under test: ',
        inlineCode('story.test.ts'),
        ' for Story tests (which drive ',
        inlineCode('update'),
        ') and ',
        inlineCode('scene.test.ts'),
        ' for Scene tests (which drive the rendered view). The name describes how the test works, not a source file, so it stays correct whether ',
        inlineCode('update'),
        ' and ',
        inlineCode('view'),
        ' live in ',
        inlineCode('main.ts'),
        ' or in their own files. When one folder holds more than one test of a kind (sibling pages, component variants), prefix with the subject, like ',
        inlineCode('login.story.test.ts'),
        '. Because a Scene always runs from the root ',
        inlineCode('update'),
        ' and ',
        inlineCode('view'),
        ', one root-level ',
        inlineCode('scene.test.ts'),
        ' covers the whole app. A large suite can split by flow into ',
        inlineCode('checkout.scene.test.ts'),
        ', ',
        inlineCode('cart.scene.test.ts'),
        ', and so on, each still entering through the root view rather than testing a sub-view in isolation.',
      ),
