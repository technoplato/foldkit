...
- **Key siblings that share a tag but play different semantic roles**, even within one layout. Previous/Next buttons at the same position, state-machine branches rendering the same `<div>` for Idle/Loading/Success/Error, etc. Without keys, snabbdom patches the same element across roles — and if there's a `transition` class, you'll see the change animate instead of swap. **Diagnostic: an unexpected CSS transition is almost always a missing key.**
- **Key list items by a stable model identifier**, never by array position. `entry.id` not `index`. Positional diffing produces correct-looking output after a mid-list remove, but snabbdom's sense of identity is wrong: it thinks row N "became" row N-1. Fine for read-only text rows, fragile for anything with focus, animation, transition, or input state.
- **Key conditional inserts between stable siblings.** When rendering `[a, ...(cond ? [b] : []), c]`, give each of a/b/c a key. The 4-pointer diff usually handles this via sel matching, but that's implicit behavior — explicit keys are free insurance and survive tag-coincidence bugs.
